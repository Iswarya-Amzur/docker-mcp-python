...
def dockerize_project(project_root: str) -> str:
    ...
    **NEW: Automatically builds and starts containers after dockerization!**
    After creating the Docker configuration, this tool will:
    1. Build all container images
    2. Start all services
    3. Wait for services to be ready
    4. Provide URLs to access your running application
    
    The application will be ready for immediate testing with test_application_e2e().
    
    Args:
        project_root: Root directory of the project containing backend/frontend folders
    
    Returns:
        Summary of dockerization process including created files, build status, and running service URLs
    """
    try:
        logger.info(f"Dockerizing full project at {project_root}")
        # auto_start=True means it will build and start containers automatically
