...
class ProductionServiceDetector:
        ...
        def scan_recursive(path: Path, depth: int = 0, max_depth: int = 5):
            ...
            for item in path.iterdir():
                ...
                if item.is_dir() and not item.name.startswith('.'):
                    ...
                    # Validate if this is truly a service directory (not just utility folder)
                    if not self._is_valid_service_directory(item):
                        # Continue scanning deeper even if this isn't a service
    ...
    def _is_valid_service_directory(self, service_path: Path) -> bool:
        """Validate if a directory is truly a service, not just a utility folder."""
        path_name = service_path.name.lower()
        
        # Exclude common non-service directories
        excluded_dirs = [
            'src', 'source',  # Source code subdirectories (not root services)
            ...
            'docker-mcp', 'docker_mcp', 'docker_tools'
        ]
        
        if path_name in excluded_dirs:
            return False
        
        # Check for key service indicators
        ...
        has_gradle = (service_path / 'build.gradle').exists()
        has_go_mod = (service_path / 'go.mod').exists()
        has_cargo = (service_path / 'Cargo.toml').exists()
        
        # Must have at least one dependency/package manager file
        has_dependency_file = any([
            has_package_json, has_requirements, has_pyproject,
            has_pom, has_gradle, has_go_mod, has_cargo
        ])
        ...
        entry_points = [
            'main.py', 'app.py', 'server.py', 'api.py', 'manage.py', 'wsgi.py', 'asgi.py',
            'main.js', 'app.js', 'server.js', 'index.js', 'index.ts', 'server.ts',
            'main.go', 'app.go', 'server.go', 'main.rs',
            'index.html', 'App.tsx', 'App.jsx', 'main.tsx', 'main.jsx',
            'main.ts', 'app.ts'
        ]
        
        # Check root directory
        has_entry_point = any((service_path / ep).exists() for ep in entry_points)
        
        # Check common source directories
        for src_dirname in ['src', 'app', 'lib', 'server', 'api']:
            src_dir = service_path / src_dirname
            if src_dir.exists() and src_dir.is_dir():
                has_entry_point = has_entry_point or any((src_dir / ep).exists() for ep in entry_points)
                if has_entry_point:
                    break
        
        # If has package.json with scripts or requirements.txt with dependencies, consider valid
        # even if no obvious entry point (could be library or has custom entry)
        if has_dependency_file and not has_entry_point:
            # Check if package.json has start/dev script (indicating it's runnable)
            if has_package_json:
                try:
                    import json
                    with open(service_path / 'package.json', 'r') as f:
                        ...
                        # Has start, dev, or serve script = valid service
                        if any(script in scripts for script in ['start', 'dev', 'serve', 'build']):
                            has_entry_point = True
                except:
                    pass
            
            # For Python, if has requirements.txt with web frameworks, consider valid
            if has_requirements or has_pyproject:
                has_entry_point = True  # Python services often have custom entry points
    ...
    def _analyze_potential_service(self, service_path: Path):
        ...
        # Validate if this is truly a service directory
        if not self._is_valid_service_directory(service_path):
            return
